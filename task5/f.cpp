...
int main(int argc, char* argv[]) {
	...
	while (index < 1)
	{
		if (device.get_capture(&capture)) {
			...
			pcl::io::savePLYFile("E:\\c\\workinVS\\cv3\\image\\fusedCloud.ply", *cloud); //将点云数据保存为 ply 文件
			pcl::io::savePCDFile("E:\\c\\workinVS\\cv3\\image\\fusedCloud.pcd", *cloud); //将点云数据保存为 ply 文件
		}
		else {
	...
	char strfilepath[256] = "E:\\c\\workinVS\\cv3\\image\\fusedCloud.pcd";
	//打开点云文件
